...
class Root:
    ...
    def __init__(self, obj_func=None, lb=None, ub=None, problem_type=0, _mat__=None, verbose=False, kwargs=None):
        """
        Parameters
        ----------
        obj_func : function
    ...
    def create_solution(self, minmax=0, iter=0):
        ...
        positionShape = [int(a) for a in position]
        position = array(positionShape,dtype=int)
        fitness = self.get_fitness_position(position=position, minmax=minmax)
        showPop_init = [iter + 1] + positionShape
        showPop_init.append(fitness)
        self._mat__[self.problem_type]['pop'].insert('', 'end', value=showPop_init)
    ...
    def get_fitness_position(self, position=None, minmax=0):
        ...
        return self.obj_func(position, self.problem_type)
    ...
    def amend_position_random(self, position=None):
        ...
        for t in range(self.problem_size):
            if position[t] < self.lb[t] or position[t] > self.ub[t]:
                position[t] = randint(self.lb[t], self.ub[t])
        return position
